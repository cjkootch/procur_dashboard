...
export async function* streamAgentTurn(
  ...
  try {
    ...
    // run inside a sandboxed code-execution container. Once the model
    // uses one, subsequent requests in the same conversation MUST echo
    // back the container id — otherwise the API returns
    // "container_id is required when there are pending tool uses
    // generated by code execution with tools." We capture the id from
    // each response and thread it forward through the loop.
